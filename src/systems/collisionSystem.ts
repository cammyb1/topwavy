import type { Entity, System, World } from "@jael-ecs/core";
...
export default function CollisionSystem(world: World): System {
  ...
  return {
    priority: PRIORITY_LIST.PHYSICS,
    update() {
      // Collisions
      RapierEngine.onCollisionDrain((handle1, handle2, started) => {
        ...
        if (entityRecieving && entityColliding) {
          const rigidbody = entityRecieving.get<RigidBody & RBUserdataEvents>(
            "rigidbody",
          );
